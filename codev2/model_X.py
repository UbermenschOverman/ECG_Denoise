...
class HybridSTFT_LIRA(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        # Bottleneck có dilation=2 để tăng receptive field
        self.bott = BottleneckDS(C, dilation=2)

        # Decoder: 2 mức
        self.up1 = UpBlock(C); self.d1 = BottleneckDS(C)
        self.up2 = UpBlock(C); self.d2 = BottleneckDS(C)

        self.head = nn.Conv2d(C, output_channels, 3, padding=1)
...
if __name__ == "__main__":
    model = HybridSTFT_LIRA(input_channels, output_channels)
